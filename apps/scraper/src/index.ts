...
import { runTelegramMonitorLoop } from "./services/telegram-monitor";
...
const app = new Elysia()
  ...
  .onBeforeHandle(async ({ request, set }) => {
    ...
    if (!isPublic) {
      ...
      const isServiceAuth =
        serviceToken && authHeader === `Bearer ${serviceToken}`;
...
httpServer.listen(config.port, () => {
  ...
  if (config.telegram.monitorEnabled && config.telegram.botToken) {
    LiveRuntime.runFork(
      Effect.gen(function* () {
        yield* Effect.logInfo("Telegram monitor loop started");
        yield* runTelegramMonitorLoop;
      }),
    );
  } else {
    log.info(
      "Telegram",
      "Monitor disabled (set TELEGRAM_BOT_TOKEN and TELEGRAM_MONITOR_ENABLED=true)",
    );
  }
});
