...
interface StatCardProps {
  ...
  icon: "devices" | "html" | "code" | "sparkle" | "check" | "x" | "discover";
  sublabel?: string;
...
function StatCard(props: StatCardProps) {
  const colorClasses = {
    indigo: {
      ...
      value: "text-indigo-700 dark:text-indigo-50",
      icon: "text-indigo-500/80 dark:text-indigo-400/80",
    },
    slate: {
    ...
    cyan: {
      ...
      value: "text-cyan-700 dark:text-cyan-50",
      icon: "text-cyan-500/80 dark:text-cyan-400/80",
    },
    violet: {
      glow: "bg-violet-500/8",
      border: "border-violet-500/20 hover:border-violet-500/40",
      text: "text-violet-600 dark:text-violet-400",
      value: "text-violet-700 dark:text-violet-50",
      icon: "text-violet-500/80 dark:text-violet-400/80",
    },
    emerald: {
      glow: "bg-emerald-500/8",
      border: "border-emerald-500/20 hover:border-emerald-500/40",
      text: "text-emerald-600 dark:text-emerald-400",
      value: "text-emerald-700 dark:text-emerald-50",
      icon: "text-emerald-500/80 dark:text-emerald-400/80",
    },
    rose: {
      glow: "bg-rose-500/8",
      border: "border-rose-500/20 hover:border-rose-500/40",
      text: "text-rose-600 dark:text-rose-400",
      value: "text-rose-700 dark:text-rose-50",
      icon: "text-rose-500/80 dark:text-rose-400/80",
    },
  };
  ...
  const icons = {
    ...
    discover: (
      <svg class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
        ...
        <path d="M11 8v6M8 11h6" stroke-linecap="round" opacity="0.6" />
      </svg>
    ),
  };
    ...
    <Show when={props.stats}>
      <div class="space-y-4">
        ...
        <div class="flex flex-col lg:flex-row items-stretch gap-3">
          ...
          <Show when={props.scrapeStats?.discovered}>
            <div class="lg:w-36 shrink-0">
              <StatCard
                label="Discovered"
                value={props.scrapeStats!.discovered!}
                color="emerald"
                icon="discover"
                sublabel="Auto-found"
                onClick={() => props.onFilterChange("discovered")}
                active={props.activeFilter === "discovered"}
              />
            </div>
          </Show>
