...
function Logo() {
  return (
    <svg viewBox="0 0 36 36" class="w-9 h-9" xmlns="http://www.w3.org/2000/svg">
      <defs>
...
export function Header(props: HeaderProps) {
  ...
  const navItems = [
    ...
    { id: "telegram", label: "Telegram", href: "/telegram", icon: "database" },
    {
      id: "yandex-links",
      label: "Yandex Links",
      href: "/yandex-links",
        ...
        <div class="flex items-center justify-between h-16">
          ...
          <div class="flex items-center gap-3">
            ...
            <Show when={props.status !== undefined}>
              ...
              >
                <span
                  class={`h-1.5 w-1.5 rounded-full ${props.status === "Connected" ? "bg-emerald-500 dark:bg-emerald-400" : "bg-rose-500 dark:bg-rose-400"}`}
                />
                {props.status}
              </div>
            </Show>
              ...
              <div class="px-3 flex items-center justify-between">
                <Show when={props.status !== undefined}>
                  <div class="flex items-center gap-2 text-sm text-zinc-600 dark:text-slate-400">
                    <span
                      class={`h-2 w-2 rounded-full ${props.status === "Connected" ? "bg-emerald-500 dark:bg-emerald-400" : "bg-rose-500 dark:bg-rose-400"}`}
                    />
                    {props.status}
                  </div>
                </Show>
